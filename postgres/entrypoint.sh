...
# Run database setup after postgres starts (meaning, this runs in the background)
{
  ...
  flyway -user=$POSTGRES_USER -password=$POSTGRES_PASSWORD -url=jdbc:postgresql://localhost:5432/archiver -locations=filesystem:///postgres/archiver/migrations migrate
